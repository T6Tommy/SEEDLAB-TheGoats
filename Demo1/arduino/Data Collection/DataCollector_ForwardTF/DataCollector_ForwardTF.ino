...
void loop() {
  ...
  if (PWM_value > 204) { // limited to 80%
    PWM_value = 204;
  }
  if (PWM_value < -204) { // limited to 80%
    PWM_value = -204;
  ...
  current_pos = old_pos + ((changeL + changeR) / 2); // we take the average of the 2 wheeles movement for more accurate data

  if (millis() > 850 & millis() < 4000) {
